  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <nav className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5 sm:pt-5">
        <div className="nav-float mx-auto max-w-7xl overflow-hidden rounded-2xl border border-white/20 bg-white/[0.09] shadow-[0_18px_55px_rgba(0,0,0,0.32),inset_0_1px_0_rgba(255,255,255,0.2)] backdrop-blur-2xl backdrop-saturate-150">
      ...
      <section id="process" className="bg-paper px-6 py-24 lg:px-10 lg:py-32"><div className="mx-auto max-w-7xl"><div className="mb-14 flex items-end justify-between border-b border-ink/10 pb-5"><div><p className="mb-4 font-mono text-xs uppercase tracking-[0.24em] text-olive">The process</p><h2 className="text-4xl font-semibold tracking-[-0.04em] text-ink sm:text-6xl">Simple by design.</h2></div><ChevronDown className="size-7 text-olive" /></div><div className="grid gap-10 md:grid-cols-4 md:gap-5">{process.map(([number, line1, line2], index) => <div key={number} className="process-step relative"><div className="mb-6 flex items-center gap-4"><span className="flex size-14 items-center justify-center rounded-full bg-ink font-mono text-lg text-lime">{number}</span>{index < 3 && <div className="hidden h-px flex-1 bg-ink/15 md:block" />}</div><p className="text-xl font-semibold leading-tight text-ink">{line1}<br />{line2}</p></div>)}</div></div></section>
